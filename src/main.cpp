/* Programa para monitoramento de Extrusoras de Polimeros
   SOFTWARE CONVERSOR DE DADOS -> S.C.D.D
   Criação data: 04-08-2023
   Termino data: 29-02-2024
   Autor:
  -Pericles Gomes Da Silva  
*/

#include <SPI.h> //(PAUL STROFFREGEN)
#include <Ethernet.h> //ARDUINO-LIBRARIES
#include <SdFat.h> //  ARDUINO REFERENCE
#include <DS3232RTC.h> // https://github.com/JChristensen/DS3232RTC
#include "RTCDS1307.h" // ARDUINO REFERENCE
#include <ArduinoJson.h>// ARDUINO REFERENCE
#include <ArduinoHttpClient.h>// ARDUINO REFERENCE
#include <avr/wdt.h> // for self reset ARDUINO REFERENCE
#include "EEPROM.h"
...
include <SDU.h>                          //  SD bootloader'
const char *BIN_FILENAME = "UPDATE.BIN"; 
#endif
#ifdef ARDUINO_ARCH_AVR
const char *BIN_FILENAME = "firmware.bin"; //fIRMWARE PARA ATUALIZAR ON-LINE
#endif

const short VERSION = 1.5;
...
//#####################################################  #
// Variavel para ativar pulsos de teste                 #
//                                                       #
int nTeste    = 0;   // 0 = Normal -  1 = Teste          #
int nVelTeste = 3.6; // 3.6 * Pulsos do sensor = RPM     #
//                                                       #
//                                                       #
//########################################################
...
String cTurnoAInicio = "05:50:01";// Turnos 
String cTurnoBInicio = "14:10:01";// podem ser alterados 
String cTurnoCInicio = "22:30:01";//conforme necessidade
  ...
  if (nFalhasAnt != nFalhas || nInicio == 0 || cOP != cOPAnt || nFimProducao != nFimProducaoAnt || nFimBobina != nFimBobinaAnt)
  {
    ...
    if (fArquivo)
    {
      ...
      else
      {
        ...
        Dados["PULSOS_MES"]    = nBatidasMesAnt;
        Dados["PULSOS_OP"]     = nBatidasOPAnt;
        Dados["PULSOS_BOBINA"] = nBatidasBobinaAnt;
        Dados["OP"]             = cOP;
      ...
      Dados["FALHA"]          = nFalhas;
      Dados["RPM"]            = nRPM;
      Dados["PULSOS"]        = nBatidas;
      Dados["PULSOS_MES"]    = nBatidasMes;
      Dados["PULSOS_OP"]     = nBatidasOP;
      Dados["PULSOS_BOBINA"] = nBatidasBobina;
      Dados["OP"]             = cOP;
      Dados["FimProducao"]    = nFimProducao;
      Dados["FimBobina"]      = nFimBobina;
      Dados["Leitura"]        = "C";
...
void setup()
{
  ...
  Serial.println("#####################################|");
  Serial.println("|                                    |");
  Serial.println("|  EXTRUSORA" + String(VERSION) + "  |");
  Serial.println("|                                    |");
  Serial.println("#####################################");
...
void Guarda_Dados()
{
  ...
  if(SD.exists("lastval.txt")){
    ...
    {    
      ...
      Dados["OP"]            = cOP;
      Dados["Pulsos"]       = nBatidas;
      Dados["PulsosMes"]    = nBatidasMes;
      Dados["PulsosOP"]     = nBatidasOP;
      Dados["PulsosBobina"] = nBatidasBobina;
      Dados["FimBobina"]     = nFimBobina;
      Dados["FimProducao"]   = nFimProducao;
...
//                      LeituraMaquina();
          ...
          //-------------------------------------------- PROGRAMAÇÂO DE EXTRUSORA DE POLIMERO --------------------------------------------
          ...
          {                                                            // Zera os valores abaixo
            unsigned long Posicao1 = readString.indexOf("VERSAO", 1.5);  // pega a posição do = antes do valor
            unsigned long Posicao2 = readString.indexOf("VERSAO") + 1.6; // pega a posição do fim depois do valor
            if (readString.substring(Posicao1, Posicao2) == "VERSAO")
            {
              ...
              client.println("    |                                     |");
              client.println("    |       -----------------------       |");
              client.println("    |       S.C.D.D" + String(VERSION) + "|");
              client.println("    |       -----------------------       |");
              client.println("    |                                     |");
          ...
          {
            unsigned long Posicao1 = readString.indexOf("MAQUINA", 1.6);   // pega a posição do = antes do valor
            unsigned long Posicao2 = readString.indexOf("MAQUINA") + 1.7; // pega a posição do fim depois do valor
